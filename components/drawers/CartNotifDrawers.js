...
export default function CartNotifDrawers({ cartOpen, setCartOpen, notifOpen, setNotifOpen }) {
  ...
  const notifications = [
    { icon: "✓", bg: "#dcfce7", color: "#16a34a", title: "Compra confirmada", desc: "Pack de Prompts WhatsApp adquirido.", time: "Agora mesmo", unread: true },
    { icon: "🔔", bg: "#dbeafe", color: "#2563EB", title: "Nova avaliação", desc: "Sua solução recebeu uma avaliação 5★", time: "2h atrás", unread: true },
    { icon: "📦", bg: "#ffedd5", color: "#f97316", title: "Solução aprovada", desc: "Agente de Atendimento foi aprovado.", time: "Ontem", unread: false },
    { icon: "ℹ", bg: "#f3f4f6", color: "#6b7280", title: "Bem-vindo à WePrompt", desc: "Complete seu perfil para começar.", time: "3 dias atrás", unread: false },
  ];
    ...
    <>
      ...
      {cartOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999 }}>
          ...
          <div style={{ position: "absolute", top: 0, right: 0, height: "100vh", width: 420, background: "white", boxShadow: "-8px 0 32px rgba(0,0,0,0.12)", display: "flex", flexDirection: "column", zIndex: 1 }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 18, fontWeight: 700, color: "#111827" }}>Carrinho</span>
              <button onClick={() => setCartOpen(false)} style={{ fontSize: 22, color: "#6b7280", cursor: "pointer", background: "none", border: "none", lineHeight: 1 }}>×</button>
            </div>
            <div style={{ flex: 1, padding: "16px 24px", overflowY: "auto" }}>
              {cartItems.map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 12, padding: "16px 0", borderBottom: "1px solid #f3f4f6", alignItems: "center" }}>
                  <div style={{ width: 56, height: 56, borderRadius: 8, background: item.gradient, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }} />
                  <div style={{ flex: 1 }}>
                  ...
                  <button style={{ fontSize: 18, color: "#9ca3af", cursor: "pointer", background: "none", border: "none" }}>×</button>
                </div>
              ))}
            </div>
      ...
      {notifOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999 }}>
          ...
          <div style={{ position: "absolute", top: 0, right: 0, height: "100vh", width: 380, background: "white", boxShadow: "-8px 0 32px rgba(0,0,0,0.12)", display: "flex", flexDirection: "column", zIndex: 1 }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                ...
                <button onClick={() => setNotifOpen(false)} style={{ fontSize: 22, color: "#6b7280", cursor: "pointer", background: "none", border: "none", lineHeight: 1 }}>×</button>
              </div>
            </div>
            <div style={{ flex: 1, padding: "8px 0", overflowY: "auto" }}>
              {notifications.map((n, i) => (
                <div key={i} style={{ display: "flex", gap: 12, padding: "14px 24px", borderBottom: "1px solid #f3f4f6", cursor: "pointer", background: n.unread ? "#fafbff" : "white", position: "relative" }}>
                  ...
                  {n.unread && <div style={{ width: 8, height: 8, borderRadius: 999, background: "#2563EB", flexShrink: 0, marginTop: 4 }} />}
                </div>
              ))}
            </div>
